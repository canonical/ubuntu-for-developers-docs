...
This guide outlines how to install the Python interpreter and developer tooling on Ubuntu. Python is an ubiquitous, object-oriented scripting language with an extensive ecosystem.
...
1. Install the {pkg}`pyenv` package:
...
1. Configure the shell environment. To ensure `pyenv` works reliably in all environments, configure both your interactive shell (`~/.bashrc`) and your login shell (`~/.profile`).
